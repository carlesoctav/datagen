...
Generates responses from two models, then compares them head-to-head using ThreadPoolExecutor.
"""
...
REASONING_SYSTEM_PROMPT = """You are a helpful reasoning assistant that always begins your response with <reasoning>, followed by a chain of thought that plans out your response, then </reasoning> and your actual response in <answer> </answer>."""


async def generate_response(
    model: str, base_url: str, prompt: str, idx: int, system_prompt: str | None = None
) -> tuple[int, dict]:
...
async def generate_all_responses(
    ...
    async def limited_generate(idx: int, prompt: str):
        async with semaphore:
            return await generate_response(model, base_url, prompt, idx, system_prompt)
...
async def main_async(args):
    """Main async function."""
    print("=" * 60)
    print("  Pairwise Comparison (Alpaca Eval Style)")
    print("=" * 60)
    ...
    system_1 = REASONING_SYSTEM_PROMPT if args.reasoning_1 else None
    system_2 = REASONING_SYSTEM_PROMPT if args.reasoning_2 else None

    print(f"\nGenerating responses from {args.model_1}...")
    responses_1 = await generate_all_responses(
        args.model_1, args.base_url_1, prompts, f"Model 1 ({args.model_1})", system_1
    )

    print(f"\nGenerating responses from {args.model_2}...")
    responses_2 = await generate_all_responses(
        args.model_2, args.base_url_2, prompts, f"Model 2 ({args.model_2})", system_2
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Pairwise comparison of two models (Alpaca Eval style)"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--reasoning_1",
        action="store_true",
        help="Use reasoning system prompt for model 1",
    )
    parser.add_argument(
        "--reasoning_2",
        action="store_true",
        help="Use reasoning system prompt for model 2",
    )
