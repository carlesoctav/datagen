...
def write_all_same_examples():
    results_path = "comparison_notebook_vm/results.json"
    ...
    for target_source in sources:
        output_file = f"{target_source}_same.md"
        print(f"Writing {target_source} examples to {output_file}...")

        with open(output_file, "w") as f:
            f.write(
                f"# {target_source.capitalize()} Comparison: mm-01 (Model 1) vs random-test (Model 2)\n\n"
            )
            ...
            for i, comp in enumerate(comparisons):
                ...
                if source == target_source:
                    ...
                    f.write(
                        f"MODEL 2 (random-test) {'[WINNER]' if winner_idx == 1 else ''}\n"
                    )
                    f.write(
                        "================================================================================\n"
                    )
